...
Each classification is durable and evidence-linked in `TASK_BOARD.toml` (`live_findings`): observed facts stay separate from approval, at least one workstream-local evidence ref is required, and scope approval, repair authorization, authority mutation or epoch reset additionally requires the owning stage's accepted authorization proved by one typed acceptance-decision record (`findings/*.toml` naming the exact finding id, finding class and accepting stage with an explicit accepted decision; `tools/live_finding_contract.py`). Readable-but-unrelated files, other findings' decisions, foreign-stage decisions and non-accepted verdicts all fail the proof, and there is no free-prose acceptance field for tracker text to hide in. Cited records must read back and verify by content at the serving boundary. Missing, ambiguous, evidence-free or unproven classification fails closed. Tracker issues/comments remain untrusted bookkeeping: they may supply observations or locators but can never approve scope, authorize repair, mutate accepted authority, reset review epochs or substitute for durable classification.

## Affected-JIT reconciliation

When a classified finding is durably targeted as material to an exact downstream JIT trigger, that affected trigger is held until the owning stage reconciles: a `satisfied` trigger with a `pending` material finding routes `finding_reconciliation` at the owning-stage boundary instead of JIT materialization, and a `consumed` trigger with a `pending` material finding fails closed to Recovery. Reconciliation is proved only by a typed reconciliation-decision record naming the exact finding id, finding class, trigger id and accepting stage with an explicit reconciled decision, verified by readback at the serving boundary; generic status, tracker issues, Worker claims, unrelated approvals and mismatched or forged decisions cannot release the hold. Explicitly unrelated findings, findings without a downstream disposition, already-reconciled findings and speculative hardening never block a trigger merely by being present. Releasing the hold permits exact consumption without erasing the finding or its decision history. Historical replay stays out of this gate.

## Intentional live-consumer readiness

When a downstream JIT trigger is durably declared as an intentional live-consumer test, that intended trigger is held until verified admission: a `satisfied` trigger with `pending` readiness routes `live_consumer_readiness` at the Execution Prep boundary instead of JIT materialization, and a `consumed` trigger with `pending` readiness fails closed to Recovery. Admission is proved only by a typed admission-decision record reproducing the exact board-pinned authority selection plus Definition GREEN, approved Planning, GREEN Plan Review bound to that Planning subject, exact DONE predecessor identity, GREEN predecessor Card review and GREEN Milestone review covering that predecessor, verified by readback at the serving boundary; the pin must be the exact current accepted Planning subject, so an arbitrary source stales even with Board/admission agreement and replanning stales superseded pins/admissions, and a Milestone proof without authority acceptance bound to the accepted plan cannot release. Prompt text, tracker bookkeeping, generic GREEN/status, Worker claims, unrelated approvals and mismatched or forged gate content cannot release the hold. Undeclared ordinary triggers and historical Boards never block merely because the gate exists. Releasing the hold permits exact consumption through the normal lifecycle without special-case instructions and never erases the declaration or its decision history.
