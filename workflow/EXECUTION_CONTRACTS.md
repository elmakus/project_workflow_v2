...
`obligation_id` is content-derived from the registered rule, exact role, exact subject and current relevant freshness fingerprint. The freshness material includes a deterministic fingerprint of the mechanically determining registered-rule metadata (rule ID, precedence, predicate, declared input paths and outcome) plus a minimal deterministic projection of that rule's declared canonical inputs. Scalar inputs retain their exact value. Collection-valued inputs are normalized to only the identity/state facts that can affect the selected mechanical obligation: for the one-READY-Card rule this is the READY/IN_PROGRESS/BLOCKED Card identity sets, while the all-DONE rule uses the canonical Card identity/status set. Record ordering and unrelated Card metadata are excluded. The production kernel derives both the rule fingerprint and this input projection from the registry/current canonical state, and derives the semantic role from the rule's registered owner module. Callers cannot substitute any of them. Explanatory prose is not hashed as a mechanical input. Equivalent determining inputs serialize byte-identically, while a material registered-rule change invalidates the old obligation. Unrelated repository state is excluded from the fingerprint rather than hashed for convenience.
...
`PolicyKernel.compile_obligations()`, `validate_results()`, and `reconcile()` are the M02 entry points. Compilation accepts only an existing registered `route` rule ID that matches current canonical state; matching `stop` or `recovery` decisions are not executable obligations. It binds the semantic role to the registered owner module and derives both the mechanically determining rule fingerprint and the minimal freshness projection of the rule's declared canonical inputs itself; it does not create a second rule vocabulary or general workflow DSL. Reconciliation refreshes both the registered rule fingerprint and that minimal input projection before recomputing freshness.
