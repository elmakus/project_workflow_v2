...
Convergence-aware PWv2.1 attempts bind a stable `review_scope` (`card`, `milestone`, or `final`) and `review_epoch`. Ordinary implementation, test and finding repair stays inside the same epoch. A changed epoch is legal only after a material accepted authority/acceptance redesign and records both a non-empty durable `epoch_reset_basis` and an exact immutable `epoch_reset_subject` of class `accepted_redesign` that binds the accepted authority path or exact acceptance surface; repair/result subjects and convenience prose cannot reset review accounting.
...
Each material defect class also has a default ceiling of 3 failed repair→closure-verification rounds in the same review epoch. A failed round is derived from a distinct repaired content state that receives RED closure verification: the reviewed repository/path/blob must differ from the source discovery content, and commit-only movement of the same content does not create another repair round. Repeated verification attempts over the same repaired content count only once. Reaching either the applicable discovery ceiling or a per-class closure-failure ceiling switches the lifecycle into Main convergence/root-cause analysis; it never turns RED into GREEN and does not authorize another ordinary discovery loop.
